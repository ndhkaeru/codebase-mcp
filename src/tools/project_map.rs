...
fn execute_blocking(args: Value) -> Result<Value> {
    ...
    Ok(json!({
        ...
        "limit_reached": truncated_directory_count > 0,
        "limit_reason": if truncated_directory_count > 0 { Some("max_children_per_dir") } else { None },
...
fn build_project_map_from_index(
    ...
    let indexed_entry_count = entries.len();
