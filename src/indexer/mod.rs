...
pub fn query_tantivy_content_candidates(
    ...
    for path in search_paths {
        ...
        let paths = search_result
            .paths
            .into_iter()
            .filter(|candidate| candidate_belongs_to_search_path(candidate, &canonical_path))
            .collect::<Vec<_>>();
        if !paths.is_empty() {
            out.extend(paths);
        }
...
fn refresh_tantivy_zone(
    ...
    let _writer_guard = writer_lock
        .lock()
        .map_err(|_| "tantivy_writer_lock_poisoned".to_string())?;
    let store = open_store(workspace_root)?;
...
fn content_zone_ready(workspace_key: &str, zone: &str) -> bool {
    INDEX_RUNTIMES
        ...
        .and_then(|guard| guard.get(workspace_key).cloned())
        .is_some_and(|state| {
            state.content_index_status != "error"
                && state
                    .content_index_zones
                    .iter()
                    .any(|existing| existing == zone)
...
fn recalc_content_totals_for_zones(
    ...
    for zone in zones {
        let allow_third_party = zone == "third_party" || zone.starts_with("third_party/");
        for entry in indexed_entries_for_content_zone(workspace_key, zone, workspace_root)? {
            if content_policy_allows(&entry, &limits, allow_third_party) {
                total_files += 1;
                if !zone.is_empty() {
...
fn index_storage_root() -> PathBuf {
    if let Some(custom_dir) = env_var_os(&["codeloupe_mcp_INDEX_DIR"]) {
...
fn tantivy_enabled() -> bool {
    std::env::var("codeloupe_mcp_TANTIVY_ENABLED")
        .ok()
