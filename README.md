...
`codebase-mcp` is a high-performance, local-first MCP server for codebase exploration and safe filesystem workflows. It runs over `stdio`, exposes 25 tools, and is designed for agents that need precise reads, search, code intelligence, structured edits, and compact responses from real repositories.
...
- Supports safe writes with structured errors.
- Inspects archives and workspace structure.
...
- Workflows that benefit from structured local file edits.
...
- Mutating tools return structured write metadata for auditing.
...
- Tool calls default to a 60 second timeout. Clients may request longer calls with `timeout_seconds`; values are capped at 600 seconds.
