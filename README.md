...
- **npm/npx**: `npm.yml` publishes `@ndhkaeru/codebase-mcp`. It waits for the matching GitHub Release binaries, bundles them into `packages/npm/native/`, and publishes the wrapper when `NPM_TOKEN` is configured.

Release checklist:

1. Update versions in `Cargo.toml`, `Cargo.lock`, and `packages/npm/package.json`.
...
6. Smoke test `npx -y @ndhkaeru/codebase-mcp@<version>`.
